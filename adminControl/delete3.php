...
if(isset($_REQUEST['submit']))
{
   ...
   $db=@mysql_select_db('quiz',$conn)or die("could not select database");
    ...
    $result=mysql_query("select * from questions where category ='{$category}'",$conn)or die('query execution failed'.mysql_error());
    echo "<h4><center><p class='alert alert-info center'><strong>Choose Question to Delete</strong></p><center></h4>";
    echo "</br>";
    echo "<table class='table table-hover'> <tr> <th> Question </th> <th> Category </th>  <th> Option A </th> <th> Option B </th> <th> Option C </th> <th> Option D </th> <th> Answer </th> <th> Task </th> </tr>";
    while($row=mysql_fetch_array($result))
    {
    echo "<tr><td> {$row['question']} </td>  <td> {$row['category']} </td> <td> {$row['a']} </td> <td> {$row['b']} </td> <td> {$row['c']} </td> <td> {$row['d']} </td> <td> {$row['ans']} </td> <td><a class='btn btn-default' href='delete2.php?id={$row['question']}'>Delete</a></td></tr>";
    }
   echo "</table>";
}
   ?>
